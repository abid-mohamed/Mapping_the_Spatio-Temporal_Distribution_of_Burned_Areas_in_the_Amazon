...
function (object, top_n = 20, num_of_features = 20) 
{
    ...
    p <- ggplot2::ggplot(ggplot2::aes(x = .shorten_model_ids(.data$model_id), 
        y = .data$feature, fill = .data$value, text = .data$text), 
        data = results) + ggplot2::geom_tile() + ggplot2::labs(fill = "Variable Importance", 
        x = "Model Id", y = "Feature", title = "Variable Importance") + 
        ggplot2::scale_x_discrete(limits = .shorten_model_ids(ordered)) + 
        ggplot2::scale_y_discrete(limits = y_ordered) + ggplot2::scale_fill_distiller(palette = "RdYlBu") + 
        ggplot2::theme_bw() + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, 
        hjust = 1), plot.margin = margin, legend.title = ggplot2::element_blank(), 
...
tic() # 77.443 sec elapsed | 275.617
...
for (zone in 1:11) {
  ...
  AZ.cv <- vfold_cv(AZ.trn, v = 3,  strata = BurntArea)
  ...
  save(AZ.nz, AZ.trn, AZ.tst, AZ.trn.folds, file = paste0(my.path,"/Amazon_selected_data/data_by_zones_folds/AZ", zone, "_data_train_test_3f.Rdata"))
  toc()
...
```{r}
options(java.parameters = "-Xmx450g")
Sys.setenv("OPENBLAS_MAIN_FREE"=1)

library(h2o)
h2o.init(ip = 'localhost', port = 50001, nthreads = 45, max_mem_size = '450g')
h2o.no_progress()
```
...
for (zone in 2:11){
  ...
  cat('- zone', zone)
  #---- Load data ----
  load(paste0(my.path,"/Amazon_selected_data/data_by_zones_folds/AZ", zone, "_data_train_test_f.Rdata"))
  ...
  save(perf.df, confusion.mtrx, max.metrics, file = paste0(path0, "/Models/GBM/GBM_AZ", zone, "_perf.Rdata"))
  # load(paste0(path0, "/Models/GBM/GBM_AZ", zone, "_perf.Rdata"))
  ...
  # Save
  save(gbm.az.predic, file = paste0(path0, "/Models/GBM/GBM_AZ", zone, "_predic.Rdata"))
...
for (zone in 3:11){
  ...
  save(perf.df, confusion.mtrx, max.metrics, file = paste0(path0, "/Models/RF/RF_AZ", zone, "_perf.Rdata"))
  # load(paste0(path0, "/Models/RF/RF_AZ", zone, "_perf.Rdata"))
  ...
  # Save
  save(rf.az.predic, file = paste0(path0, "/Models/RF/RF_AZ", zone, "_predic.Rdata"))
  # load(paste0(path0, "/Models/RF/RF_AZ", zone, "_predic.Rdata"))
...
for (zone in 1:11){
  ...
  load(paste0(path0, "/Models/XGB/XGB_AZ", zone, "_perf.Rdata"))
  
  #---- Variable importance ----
  # Retrieve variable importance
  vi.xgb.az <- h2o.varimp(xgb.az) %>% as.data.frame()
...
for (zone in 4:11){
  ...
  # Retrieve variable importance
  # h2o.varimp(glm.az)
    vi.glm.az <- h2o.varimp(glm.az) %>% as.data.frame()

  
  #---- Plot variable importance ----
  # h2o.varimp_plot(model = glm.az, num_of_features = 50)
...
for (zone in 1:11){ # zone <- 1
  ...
  for (m in Models){
    ...
    beta.m['beta', m] <- 1 / perf.df[perf.df$metrics=='Mean Per-Class Error',]$values
  ...
  p1.nz <- Map(`*`, prob.df, beta.m) %>% as.data.frame() %>% rowSums()
  # Compute the F1 performance score for each threshold 
  predicted.rocr <- ROCR::prediction(p1.nz, AZ.nz$BurntArea)
  ...
  print(c(F1= f1.max, cutoff = cutoff))
  plot(f1.perf)
  # Compute the prediction
  pred <- ifelse(p1.nz > cutoff, 1, 0)
}
```
...
nbr.z <- 11
#---- Load Amazon data ----
system.time( load(paste0(path0,"/Amazon_selected_data/AZ_norm.Rdata")) ) # 147.719 sec
setDT(AZ.norm)
#
for (zone in 4:nbr.z){ # zone <- 1
  ...
  #---- Save data ----
  save(AZ.bar, file = paste0(path0,"/Amazon_selected_data/data_by_zones/AZ", zone, "_bar.Rdata"))
  rm(AZ.nz, AZ.trn, AZ.tst, AZ.bar)
  toc()
  # AZ1_bar : __ sec elapsed
  # AZ2_bar : 357.683 sec elapsed
  # AZ3_bar : 535.238 sec elapsed
  # AZ4_bar : 396.406 sec elapsed
  # AZ5_bar : 480.9 sec elapsed
  # AZ6_bar : 444.303 sec elapsed
  # AZ7_bar : 410.457 sec elapsed
  # AZ8_bar : 895.739 sec elapsed
  # AZ9_bar : 839.074 sec elapsed
  # AZ10_bar : 816.453 sec elapsed
  # AZ11_bar : 818.304 sec elapsed
}
```
...
# perf.df$Zone <- rep(1:11, each=nbr.m); perf.df$Model <- rep(Models, nbr.z)
# 
# confusion.mtrx.lst <- max.metrics.lst <- vi.lst <- vector(mode='list', length=nbr.z*nbr.m)
# names(confusion.mtrx.lst) <- names(max.metrics.lst) <- names(vi.lst) <- c(outer(Models, as.character(1:nbr.z), paste0))
# 
# #---- Load Amazon data ----
# system.time({
...
#   system.time( load(paste0(path0,"/Amazon_selected_data/AZ_norm.Rdata")) ) # 147.719 sec
#   setDT(AZ.norm)
#   AZ.h2o <- as.h2o(AZ.norm) # 263.103 sec elapsed |
#   rm(AZ.norm); gc()
# }) # 430.061 sec

#---- Compute performance ----
tic("Total Performance ")
zone.lst <- c(7:8)
for (zone in zone.lst){ # zone <- 1
  #---- load data ----
  mask <- as.numeric(AZ.h2o["Zones"]) != zone
  AZ.bar.h2o <- AZ.h2o[mask, ] # 0 sec elapsed
  Sys.sleep(10)
  
  #---- Load test data ----
  load(paste0(path0,"/Amazon_selected_data/data_by_zones/AZ", zone, "_data_train_test.Rdata")) # AZ.nz, AZ.trn, AZ.tst () # 58.153 sec elapsed
  setDT(AZ.tst)
  AZ.tst.h2o <- as.h2o(AZ.tst)
  AZ.bar.h2o <- h2o.rbind(AZ.tst.h2o, AZ.bar.h2o) # __ sec elapsed
  Sys.sleep(10)
  ...
  for (m in Models){ # m <- 'GLM'
    ...
    vi <- h2o.varimp(model.az) %>% as.data.table()
    #---- Collect results ----
    cat("Save - ")
    Values <- c(
      az.perf@metrics$MSE,
  ...
  save(perf.df, confusion.mtrx.lst, max.metrics.lst, vi.lst, file = paste0(path0, "/Models_ens/Ens2_new/Ens2_performance_vi_", zone, ".Rdata"))
...
toc()
# load(paste0(path0, "/Models_ens/Ens3/Ens3_performance_vi.Rdata")) # perf.df, confusion.mtrx.lst, max.metrics.lst, vi.lst
```

Zone 1 , Model GLM : Load the model and data - Performance - Save - GLM1: 19.89 sec elapsed
Zone 1 , Model GBM : Load the model and data - Performance - Save - GBM1: 3559.182 sec elapsed
Zone 1 , Model XGB : Load the model and data - Performance - Save - XGB1: 1390.747 sec elapsed
Zone 1 , Model RF : Load the model and data - Performance - Save - RF1: 13807.492 sec elapsed

Zone 2 , Model GLM : Load the model and data - Performance - Save - GLM2: 29.221 sec elapsed
Zone 2 , Model GBM : Load the model and data - Performance - Save - GBM2: 3268.644 sec elapsed
Zone 2 , Model XGB : Load the model and data - Performance - Save - XGB2: 1377.19 sec elapsed
Zone 2 , Model RF : Load the model and data - Performance - Save - RF2: 12474.306 sec elapsed

Zone 3 , Model GLM : Load the model and data - Performance - Save - GLM3: 23.672 sec elapsed
Zone 3 , Model GBM : Load the model and data - Performance - Save - GBM3: 3349.325 sec elapsed
Zone 3 , Model XGB : Load the model and data - Performance - Save - XGB3: 1443.942 sec elapsed
Zone 3 , Model RF : Load the model and data - Performance - Save - RF3: 12292.447 sec elapsed

Zone 4 , Model GLM : Load the model and data - Performance - Save - GLM4: 21.7 sec elapsed
Zone 4 , Model GBM : Load the model and data - Performance - Save - GBM4: 3429.272 sec elapsed
Zone 4 , Model XGB : Load the model and data - Performance - Save - XGB4: 1404.618 sec elapsed
Zone 4 , Model RF : Load the model and data - Performance - Save - RF4: 13764.939 sec elapsed

Zone 5 , Model GLM : Load the model and data - Performance - Save - GLM5: 21.512 sec elapsed
Zone 5 , Model GBM : Load the model and data - Performance - Save - GBM5: 3336.955 sec elapsed
Zone 5 , Model XGB : Load the model and data - Performance - Save - XGB5: 1252.732 sec elapsed
Zone 5 , Model RF : Load the model and data - Performance - Save - RF5: 14081.912 sec elapsed

Zone 6 , Model GLM : Load the model and data - Performance - Save - GLM6: 21.102 sec elapsed
Zone 6 , Model GBM : Load the model and data - Performance - Save - GBM6: 3373.733 sec elapsed
Zone 6 , Model XGB : Load the model and data - Performance - Save - XGB6: 1082.018 sec elapsed
Zone 6 , Model RF : Load the model and data - Performance - Save - RF6: 13876.594 sec elapsed

Zone 7 , Model GLM : Load the model and data - Performance - Save - GLM7: 29.638 sec elapsed
Zone 7 , Model GBM : Load the model and data - Performance - Save - GBM7: 3381.968 sec elapsed
Zone 7 , Model XGB : Load the model and data - Performance - Save - XGB7: 1209.227 sec elapsed
Zone 7 , Model RF : Load the model and data - Performance - Save - RF7: 13381.724 sec elapsed

Zone 8 , Model GLM : Load the model and data - Performance - Save - GLM8: 20.145 sec elapsed
Zone 8 , Model GBM : Load the model and data - Performance - Save - GBM8: 3374.753 sec elapsed
Zone 8 , Model XGB : Load the model and data - Performance - Save - XGB8: 1131.046 sec elapsed
Zone 8 , Model RF : Load the model and data - Performance - Save - RF8: 12909.196 sec elapsed

Zone 9 , Model GLM : Load the model and data - Performance - Save - GLM9: 21.748 sec elapsed
Zone 9 , Model GBM : Load the model and data - Performance - Save - GBM9: 3422.291 sec elapsed
Zone 9 , Model XGB : Load the model and data - Performance - Save - XGB9: 1237.889 sec elapsed
Zone 9 , Model RF : Load the model and data - Performance - Save - RF9: 11988.416 sec elapsed

Zone 10 , Model GLM : Load the model and data - Performance - Save - GLM10: 22.4 sec elapsed
Zone 10 , Model GBM : Load the model and data - Performance - Save - GBM10: 3348.346 sec elapsed
Zone 10 , Model XGB : Load the model and data - Performance - Save - XGB10: 1239.392 sec elapsed
Zone 10 , Model RF : Load the model and data - Performance - Save - RF10: 12505.208 sec elapsed

Zone 11 , Model GLM : Load the model and data - Performance - Save - GLM11: 20.167 sec elapsed
Zone 11 , Model GBM : Load the model and data - Performance - Save - GBM11: 3427.842 sec elapsed
Zone 11 , Model XGB : Load the model and data - Performance - Save - XGB11: 1175.126 sec elapsed
Zone 11 , Model RF : Load the model and data - Performance - Save - RF11: 11793.657 sec elapsed
...
prepareMonthDT <- function(n.month, var00.dt){
  ...
  var.norm <- AZ_recipe %>% 
    ...
    as.data.table()
  toc() # Normalize data: 14.198 sec elapsed
  # save(var.norm, file = paste0(path0, "/Amazon_new_data/DT/amazon_dt_", sq.date[n.month], ".Rdata")) # 40.669 sec
  return(var.norm)
}
...
sq.date <- c(
  ...
  # 
  # "2001_04", "2002_04", "2003_04", "2004_04", "2005_04", "2006_04", "2007_04", "2008_04", "2009_04", "2010_04",  
  # "2011_04", "2012_04", "2013_04", "2014_04", "2015_04", "2016_04", "2017_04", "2018_04", "2019_04", "2020_04",
  # 
  # "2001_05", "2002_05", "2003_05", "2004_05", "2005_05", "2006_05", "2007_05", "2008_05", # "2009_05", 
  # "2010_05",  "2011_05", "2012_05", "2013_05", "2014_05", "2015_05", "2016_05", "2017_05", "2018_05", "2019_05", 
  # "2020_05",
  # 
  # "2001_07", "2002_07", "2003_07", "2004_07", "2005_07", "2006_07", "2007_07", "2008_07", "2009_07", "2010_07",  
  # "2011_07", "2012_07", "2013_07", "2014_07", "2015_07", "2016_07", "2017_07", "2018_07", "2019_07", "2020_07",
  
  # "2001_08", "2002_08", "2003_08", "2004_08", "2005_08", "2006_08", "2007_08", "2008_08", "2009_08", "2010_08",  
  # "2011_08", "2012_08", "2013_08", "2014_08", "2015_08", "2016_08", "2017_08", "2018_08", "2019_08", "2020_08",
  
  # "2001_10", "2002_10", "2003_10", "2004_10", "2005_10", "2006_10", "2007_10", "2008_10", "2009_10", "2010_10",  
  # "2011_10", "2012_10", "2013_10", "2014_10", "2015_10", "2016_10", "2017_10", "2018_10", "2019_10", # "2020_10",
  # 
  # "2001_11", "2002_11", "2003_11", "2004_11", "2005_11", "2006_11", "2007_11", "2008_11", "2009_11", "2010_11",  
  # "2011_11", "2012_11", "2013_11", "2014_11", "2015_11", "2016_11", "2017_11", "2018_11", "2019_11", "2020_11",
  
  # "2001_12", "2002_12", "2003_12", "2004_12", "2005_12", "2006_12", "2007_12", "2008_12", "2009_12", "2010_12",  
  # "2011_12", "2012_12", "2013_12", "2014_12", "2015_12", "2016_12", "2017_12", "2018_12", "2019_12", "2020_12"
)
...
for(n.month in sq.date) {
  ...
  #---- Mean of ensembles ----
  p1.ens.h2o[, 'p1_mean'] <- h2o.sum(p1.models.h2o[, c(2:12)], axis=1, return_frame = T)/nbr.z
...
Sys.setenv("OPENBLAS_MAIN_FREE"=1)
library(h2o)
h2o.init(ip = 'localhost', port = 50001, nthreads = 45, max_mem_size = '650g')
h2o.no_progress()
...
# v0.dt <- var.norm[, c('Zones', 'cell', 'x', 'y')]
# var.nz <- var.norm
system.time( var.h2o <- as.h2o(AZ.test) ) # 69.060 sec | 120.980 sec
p1.ens.h2o <- var.h2o[, c('x', 'y')]
p1.models.h2o <- var.h2o[, c('x', 'y')]
mtrc <- 'AUCPR'
...
for (zone in zone.lst){ # zone <- 1
  ...
  cat("\n"); toc() # Zone 
# Zone 1: __ sec elapsed
# Zone 2: __  sec elapsed
# Zone 3: __  sec elapsed
# Zone 4: __  sec elapsed
# Zone 5: __  sec elapsed
# Zone 6: __  sec elapsed
# Zone 7: __  sec elapsed
# Zone 8: __  sec elapsed
# Zone 9: __  sec elapsed
# Zone 10: __  sec elapsed
# Zone 11: __  sec elapsed
} # Total:  ~ 2.5 h

#---- Mean of ensembles ----

p1.ens.h2o[, 'p1_mean'] <- h2o.sum(p1.models.h2o[, c(3:13)], axis=1, return_frame = T)/nbr.z
